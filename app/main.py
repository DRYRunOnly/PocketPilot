...
@app.get("/profile")
def get_profile(x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    auth(x_api_key)
    return {
...
def create_month_plan(req: MonthPlanRequest, x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    auth(x_api_key)
...
def add_transactions(batch: TransactionBatch, x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    auth(x_api_key)
...
@app.post("/holdings")
def upsert_holdings(req: HoldingsUpdateRequest, x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    auth(x_api_key)
    for h in req.items:
...
def add_networth(req: NetWorthSnapshot, x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    auth(x_api_key)
    net = (req.cash_bank + req.fd_total + req.ppf_balance + req.equity_value + req.mf_value + req.other_assets) - (req.liabilities_cc + req.liabilities_loans)
...
def close_month(req: MonthCloseRequest, x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    auth(x_api_key)
...
def upsert_notion_month_page(req: NotionUpsertRequest, x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    auth(x_api_key)
